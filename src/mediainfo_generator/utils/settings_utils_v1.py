...
class SettingsOutput:
    ...
    def __init__(
        self, individualFiles: str, combinedFile: str, summaryFile: str
    ) -> None:
        self.individualFiles = individualFiles.strip().lower() in [
            "enabled",
            "yes",
            "y",
            "true",
        ...
        self.combinedFile = combinedFile.strip().lower() in [
            "enabled",
            "yes",
            "y",
            "true",
        ...
        self.summaryFile = summaryFile.strip().lower() in [
            "enabled",
            "yes",
            "y",
            "true",
...
class Settings:
    ...
    def __init__(
        self, version: str, path: SettingsPath, output: SettingsOutput
    ) -> None:
...
class SettingsManager:
    ...
    def loadFromFile(self, fileName: str, cmdLineArgs=None) -> None:
        ...
        self._config = confuse.Configuration(
            "mediainfo_generator", "mediainfo_generator"
        )
        ...
        if os.path.exists(fileName):
            self._logger.debug(
                "Loading Configuration from User Configuration: '{}'".format(fileName)
            )
