...
class MediaInfoScanner:
    ...
    def __get_media_info(self, file_path):
        ...
        media_info["format"]["filename"] = media_info["format"]["filename"].replace(
            os.path.abspath(globalSettingsMgr.settings.path.data), ""
        )
            ...
            for file in files:
                ...
                if file.lower().endswith((".mp4", ".mkv", ".avi", ".m4v", ".mv4")):
                    ...
                    if saveIndividualFile:
                        json_file_path = (
                            os.path.splitext(file_path)[0] + ".mediainfo.json"
                        )

                        if os.path.exists(json_file_path):
                            self.logger.debug(
                                f"Removing existing file: {json_file_path}"
                            )
                        ...
                        self.logger.info(
                            f'\tSaved media info for file: "{os.path.basename(file_path)}" to: "{os.path.basename(json_file_path)}"'
                        )
    ...
    def __save_media_info(self):
        self.logger.debug(
            f"Saving media info to: {globalSettingsMgr.settings.path.output}"
        )
    ...
    def __save_summary_info(self):
        self.logger.debug(
            f"Saving summary info to: {globalSettingsMgr.settings.path.output}"
        )

        output_file_path = (
            f"{globalSettingsMgr.settings.path.output}/mediainfo.summary.json"
        )
